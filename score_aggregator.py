...
def aggregate_scores(company):
    ...
    return combine_scores(company_score, company_industry_score, company_size_score, company_owner_score)

def combine_scores(company_score, industry_score, size_score, owner_score):
    ...
    total_weight = COMPANY_WEIGHT + INDUSTRY_WEIGHT + SIZE_WEIGHT + OWNER_WEIGHT
    weighted_score = (company_factor + industry_factor + size_factor + owner_factor + 0.0) / total_weight
